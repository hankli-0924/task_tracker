...
class ScheduleService:
    ...
    def recalculate_assignment_schedule(assignment: Assignment):
        ...
        new_planned_start_date_for_assignment = max(
            [end_time for end_time in [max_actual_end_time, timezone.now()] if
    ...
    def reschedule_team_member(team_member):
        ...
        assignments = Assignment.objects.filter(
            Q(team_member=team_member, task__level=1, actual_start_time__isnull=True, actual_end_time__isnull=True) |
            Q(team_member=team_member, task__level=1, need_update=True),
